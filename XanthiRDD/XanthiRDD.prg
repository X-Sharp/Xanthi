...
BEGIN NAMESPACE XanthiRDD
    ...
    CLASS XanthiRDD IMPLEMENTS IRDD
        ...
        // Good Idea ?
        PRIVATE PROPERTY SessionID AS INT GET SELF:Connection:SessionID
        ...
        PRIVATE METHOD GetRDDState( msg AS Message ) AS VOID
            //
        ...
        /// </summary>
        /// <param name="nCommand">The CommandValue that identify the command</param>
        /// <param name="param">int value as parameter</param>
        /// <param name="isOk">The result of the Command as a REF Logic</param>
        /// <returns>The result of the Command in a String form (usually a Serialized object in JSON)</returns>
        PRIVATE METHOD RecvCommand( nCommand AS CommandValue, param AS INT, isOk REF LOGIC ) AS STRING
            LOCAL msg AS Message
            LOCAL reply := NULL AS STRING
            IF SELF:SessionID == 0
                RETURN reply
        ...
        PUBLIC METHOD FieldName(nFldPos AS INT) AS STRING
            // Maybe we should store the DbStruct in the Open Reply ?
            LOCAL reply AS STRING
            LOCAL isOk := FALSE AS LOGIC
            LOCAL data := NULL AS STRING
            //
            reply := RecvCommand( CommandValue.FieldName, nFldPos, REF isOk )
            IF isOk
        ...
        PUBLIC METHOD GetField(nFldPos AS INT) AS XSharp.RDD.Support.RddFieldInfo
            ...
            //
            reply := RecvCommand( CommandValue.GetField, nFldPos, REF isOk )
            IF isOk
        ...
        PUBLIC METHOD GetValue(nFldPos AS INT) AS OBJECT
            ...
            LOCAL data := NULL AS OBJECT
            //
            reply := RecvCommand( CommandValue.GetValue, nFldPos, REF isOk )
            IF isOk
                data := JsonConvert.DeserializeObject( reply ) 
            ENDIF
            RETURN data
        ...
        PUBLIC METHOD GetValueLength(nFldPos AS INT) AS INT
            ...
            LOCAL data := 0 AS INT
            //
            reply := RecvCommand( CommandValue.GetValueLength, nFldPos, REF isOk )
            IF isOk
                data := JsonConvert.DeserializeObject<INT>( reply ) 
            ENDIF
            RETURN data
        ...
        PUBLIC METHOD RelArea(nRelNum AS DWORD) AS DWORD
            ...
            //
            reply := RecvCommand( CommandValue.RelArea, (INT)nRelNum, REF isOk )
            IF isOk
        ...
        PUBLIC METHOD RelText(nRelNum AS DWORD) AS STRING
            ...
            LOCAL data := NULL AS STRING
            //
            reply := RecvCommand( CommandValue.RelText, (int)nRelNum, REF isOk )
            IF isOk
                data := JsonConvert.DeserializeObject<STRING>( reply ) 
            ENDIF
            RETURN data
